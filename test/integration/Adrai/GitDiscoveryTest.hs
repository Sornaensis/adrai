...
tests =
  testGroup
    ...
    [ testCase "non-repository and unavailable executable are structured errors" $
      ...
      testCase "main worktree discovery is canonical from a Unicode subdirectory" $
        withSystemTempDirectory "adrai git discovery" $ \temporary -> do
      ...
      testCase "ordinary and bare-origin linked worktrees keep per-worktree git dirs and common storage" $
        withSystemTempDirectory "adrai git linked" $ \temporary -> do
          let mainRepository = temporary </> "ordinary main"
              ordinaryWorktree = temporary </> "ordinary linked"
              bareRepository = temporary </> "origin bare.git"
              bareWorktree = temporary </> "bare linked"
          initTestRepository mainRepository
          _ <- commitFile mainRepository "seed.txt" "seed"
          _ <- gitSuccess mainRepository ["worktree", "add", "-b", "feature", ordinaryWorktree, "HEAD"] BS.empty
          ordinary <- discoverRepository systemGit ordinaryWorktree
          case ordinary of
            Left problem -> assertFailure (show problem)
            Right discovered -> do
              repositoryLayout discovered @?= LinkedWorktree
              repositoryCommonIsBare discovered @?= False
              assertBool "linked git dir differs from common" (repositoryGitDir discovered /= repositoryCommonDir discovered)
              repositoryCommandDirectory discovered @?= maybe "" id (repositoryWorktreeRoot discovered)
          _ <- gitSuccess temporary ["clone", "--bare", mainRepository, bareRepository] BS.empty
          _ <- gitSuccess bareRepository ["worktree", "add", "-b", "bare-feature", bareWorktree, "main"] BS.empty
          bareLinked <- discoverRepository systemGit bareWorktree
          case bareLinked of
            Left problem -> assertFailure (show problem)
            Right discovered -> do
              repositoryLayout discovered @?= LinkedWorktree
              repositoryCommonIsBare discovered @?= True
              assertBool "bare-linked git dir differs from common" (repositoryGitDir discovered /= repositoryCommonDir discovered)
    ]
