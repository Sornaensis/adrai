...
renderHelp =
  ...
  , ""
  , ansiBold "Commands:"
  , "  " <> ansiCyan "help" <> "                          — Show this help"
  , "  " <> ansiCyan "search <query>" <> "                 — Search ADRs"
  , "  " <> ansiCyan "show <id>" <> "                    — Show ADR detail"
  , "  " <> ansiCyan "view <id> [collapsed|exploded]" <> "  — View ADR (collapsed or exploded)"
  , "  " <> ansiCyan "history [id]" <> "                 — Show operation history"
  , "  " <> ansiCyan "conflicts" <> "                    — List conflicts"
  , "  " <> ansiCyan "create --title T --body B" <> "    — Create new ADR"
  , "  " <> ansiCyan "amend <id> --title T --body B" <> " — Amend existing ADR"
  , "  " <> ansiCyan "status <id> [active|obsolete]" <> "  — Change ADR status"
  , "  " <> ansiCyan ":view collapsed|exploded" <> "       — Set view mode"
  , "  " <> ansiCyan ":mode fts|vector|hybrid" <> "        — Set search mode"
  , "  " <> ansiCyan ":at <revision>" <> "                 — Set revision"
  , "  " <> ansiCyan ":obsolete on|off" <> "               — Toggle obsolete inclusion"
  , "  " <> ansiCyan ":actor <id>" <> "                  — Set actor ID"
  , ""
